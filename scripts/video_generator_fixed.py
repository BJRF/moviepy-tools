...
def main():
    """主函数，从test.json文件读取数据并生成视频"""
    
    # 获取当前脚本目录
    current_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 只使用test.json文件
    test_file_path = os.path.join(current_dir, "test.json")
    
    # 项目根目录的输出路径
    project_root = os.path.dirname(current_dir)
    output_dir = os.path.join(project_root, "output", "video")
    
    print("🎬 开始从test.json文件生成视频...")
    print(f"📁 数据文件: {test_file_path}")
    print(f"📁 输出目录: {output_dir}")
    
    # 检查test.json文件是否存在
    if not os.path.exists(test_file_path):
        print("❌ 找不到test.json文件")
        return
    
    # 加载视频数据
    video_data = load_video_data_from_file(test_file_path)
    if not video_data:
        print("❌ 加载视频数据失败")
        return
